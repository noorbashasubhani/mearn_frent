...
exports.getAllInflows = async(req,res) => {
  try{
    const list = await Inflow.find();
...
exports.getSingleInflow = async(req,res)=>{
const { row_id }=req.params;
  try{
    const list = await Inflow.findById({_id:row_id});
    if(!list){
...
exports.deleteInflow = async (req, res) => {
  ...
  try {
    ...
    const dd = await Inflow.findByIdAndDelete(row_id);
...
exports.updateInflow = async (req, res) => {
  ...
  try {
    ...
    const updateFlow = await Inflow.findByIdAndUpdate(
      row_id,
      { name },
