...
const { row_id }=req.params;
  try{
    const list = await Tax.findById({_id:row_id});
    if(!list){
...
exports.deleteTax = async (req, res) => {
  ...
  try {
    ...
    const dd = await Tax.findByIdAndDelete(row_id);
...
exports.updateTax= async (req, res) => {
  ...
  try {
    ...
    const updateFlow = await Tax.findByIdAndUpdate(
      row_id,
      { name },
