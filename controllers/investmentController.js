...
exports.getInst = async(req,res) => {
  try{
    const list = await Investments.find();
...
exports.getSingleInst = async(req,res)=>{
const { row_id }=req.params;
  try{
    const list = await Investments.findById({_id:row_id});
    if(!list){
...
exports.updateInst = async (req, res) => {
  const { row_id } = req.params;  // Get row_id from the request parameters
  ...
  try {
    ...
    const updateFlow = await Investments.findByIdAndUpdate(
      row_id,
      { name },
